\begin{table}[htb]
  \caption{Meteorological data during the experiment period}
  \label{tbl:des1}
  \begin{adjustwidth}{-0.05\textwidth}{-0.05\textwidth}
    \begin{center}
      \resizebox{1.1\textwidth}{!}{
        \begin{tabular}{cccc}
          \hline
                  & \begin{tabular}[c]{@{}c@{}}Average Temperature\\ ($^\circ C$)\end{tabular} & \begin{tabular}[c]{@{}c@{}}total precipitation\\ ($mm$)\end{tabular} & \begin{tabular}[c]{@{}c@{}}Average daily total solar radiation\\ ($MJ \cdot m^{-2}$)\end{tabular} \\ \hline
          2021.10 & 17.6                                                                       & 156.5                                                                & 11.2                                                                                              \\
          2021.11 & 12.2                                                                       & 85.0                                                                 & 11.0                                                                                              \\
          2012.12 & 6.4                                                                        & 123.0                                                                & 9.6                                                                                               \\
          2022.01 & 3.6                                                                        & 16.5                                                                 & 10.6                                                                                              \\
          2022.02 & 3.8                                                                        & 60.0                                                                 & 13.3                                                                                              \\
          2022.03 & 10.3                                                                       & 94.5                                                                 & 15.3                                                                                              \\
          2022.04 & 15.0                                                                       & 213.0                                                                & 15.9                                                                                              \\ \hline
          \end{tabular}
      }
    \end{center}
  \end{adjustwidth}
\end{table}